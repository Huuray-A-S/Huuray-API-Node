...
The API can signal "nothing found" as HTTP 404: `orders.search()` with no match throws `HuurayNotFoundError` rather than returning an empty result. `POST /v4/Template` has been observed live both ways (see [CHANGELOG](CHANGELOG.md)): a 404 (*"There were no active templates"*) when the account had no templates, which `templates.list()` throws as `HuurayNotFoundError`, and a 200 with an empty `templates` list for an account with PDF templates but no email or SMS templates. Handle both:
...
let templates = [];
try {
  ({ templates } = await huuray.templates.list());   // can be []
} catch (err) {
  if (!(err instanceof HuurayNotFoundError)) throw err;   // 404: seen when the account had no templates
}
```
...
Found a bug, or something in this library that could be friendlier? Please [file an issue](https://github.com/Huuray-A-S/Huuray-API-Node/issues) or start a [discussion](https://github.com/Huuray-A-S/Huuray-API-Node/discussions). This repository does not accept external pull requests — see [CONTRIBUTING.md](.github/CONTRIBUTING.md) for why.
