...
- Test fixtures must contain only invented data. Never record a real API response into `test/fixtures/`.
- The library redacts voucher codes, credentials and contact details from anything it prints. If you add a field that carries value or personal data, add it to `SECRET_FIELDS` or `SENSITIVE_FIELDS` in [`src/redact.ts`](../src/redact.ts) and cover it with a test.
